...
int p_setup(int argc, char** argv, MPI_File *fha, MPI_File *fhb, int* dim){
    /* 并行读取文件 */
    int res;
    /* 文件大小 */
    long long fsizea, fsizeb;
    /* 维数 */
    int n1, n2, n3, n4;
    MPI_Status status;
    ...
    MPI_File_open(MPI_COMM_WORLD, argv[1], MPI_MODE_RDONLY, MPI_INFO_NULL, fha);
    MPI_File_open(MPI_COMM_WORLD, argv[2], MPI_MODE_RDONLY, MPI_INFO_NULL, fhb);
    MPI_File_get_size(*fha, &fsizea);
    MPI_File_get_size(*fhb, &fsizeb);
    MPI_File_read_at_all(*fha, 0, &n1, 1, MPI_INT, &status);
    MPI_File_read_at_all(*fha, sizeof(int), &n2, 1, MPI_INT, &status);
    MPI_File_read_at_all(*fhb, 0, &n3, 1, MPI_INT, &status);
    MPI_File_read_at_all(*fhb, sizeof(int), &n4, 1, MPI_INT, &status);
    /* 检查矩阵数值 */
    if (n1 <=0 || n2 <=0 || n3 <= 0 || n4 <=0 || n2 != n3) {
        printf("Matrix size error, %dx%d with %dx%d\n", n1, n2, n3, n4);
        return 1;
...
void p_scatter_matrix(MPI_File *fh, int m, int n, double *matrix, int rootp){
    /* 用于区分进行的变量 */
    int myid, numprocs;
    int i, j, k;
    int offset;
    ...
    k = 0;
    for(i = i_min; i < i_max; i++){
        offset = (i * n + j_min) * sizeof(double) + 2 * sizeof(int);
        MPI_File_read_at(*fh, offset, matrix_v + k, j_max - j_min, MPI_DOUBLE, &status);
        k += j_max - j_min;
    }
    if(DEBUG){
        printf("%d read matrix with dim %d x %d!\n", myid, ((int*)matrix)[0], ((int*)matrix)[1]);
        for(i = 0; i < i_max - i_min; i++){
            for(j = 0; j < j_max - j_min; j++){
                printf("%.4f  ", matrix_v[i * ((int*)matrix)[1] + j]);
            }
...
void p_gather_matrix(MPI_File *fh, int n1, int n3, double *C, int rootp){
    /* 用于区分进行的变量 */
    int myid, numprocs;
    int i, j, k;
    int offset;
    ...
    j_min = (myid % rootp) * maxcols;
    /* 需要处理最后一块的矩阵大小不平衡 */
    i_max = (myid / rootp + 1) * maxrows > n1 ? n1 : (myid / rootp + 1) * maxrows;
    j_max = (myid % rootp + 1) * maxcols > n3 ? n3 : (myid % rootp + 1) * maxcols;

    /* 将本进程的矩阵写到相应的位置 */
    MPI_Status status[i_max - i_min + 2];
    MPI_Request request[i_max - i_min + 2];
    k = 0;
    for(i = i_min; i < i_max; i++){
        offset = (i * n3 + j_min) * sizeof(double) + 2 * sizeof(int);
        MPI_File_iwrite_at(*fh, offset, C_v + k, j_max - j_min, MPI_DOUBLE, &request[i - i_min]);
        //MPI_Wait(&request, &status);
        k += j_max - j_min;
    }
    MPI_Waitall(i_max - i_min, request, status);
    if(DEBUG){
        printf("%d write C matrix with dim %d x %d!\n", myid, i_max - i_min, j_max - j_min);
        for(i = 0; i < i_max - i_min; i++){
            for(j = 0; j < j_max - j_min; j++){
                printf("%.4f  ", C_v[i * (j_max - j_min) + j]);
            }
...
int main(int argc, char** argv){
    ...
    read_time = MPI_Wtime();
    /* 并行打开文件, 检查矩阵尺寸,  并且将其维数存到 dim */
    if (p_setup(argc, argv, &fha, &fhb, dim)) {
        MPI_File_close(&fha);
        MPI_File_close(&fhb);
        MPI_Finalize(); // Something error during preprocessing
        exit(-1);
    ...
    /* 并行读取矩阵块 */
    p_scatter_matrix(&fha, n1, n2, A, rootp);
    p_scatter_matrix(&fhb, n2, n3, B, rootp);
    ...
    /* 并行输出到文件 */
    save_time = MPI_Wtime();
    MPI_File_open(MPI_COMM_WORLD, argv[3], MPI_MODE_CREATE | MPI_MODE_RDWR, MPI_INFO_NULL, &fhc);
    if(myid == 0){
        MPI_File_write_at(fhc, 0, &n1, 1, MPI_INT, &status);
        MPI_File_write_at(fhc, sizeof(int), &n3, 1, MPI_INT, &status);    
    }
    /* 收集计算结果 */
    //MPI_Barrier(MPI_COMM_WORLD);
    //gather_matrix((double*)(fstreamc + sizeof(int)*2), n1, n3, C, rootp);
    p_gather_matrix(&fhc, n1, n3, C, rootp);
    //MPI_Barrier(MPI_COMM_WORLD);
    MPI_File_close(&fhc);
